...
void Game::drawTexts(Canvas &canvas, const char *title, const char *msg) {
    ...
    drawShadowText(canvas, (ScreenWidth - canvas.textWidth(title)) / 2, 30, Canvas::Color::Bright, title);

    canvas.setFont(Font::Tiny);
    drawShadowText(canvas, (ScreenWidth - canvas.textWidth(msg)) / 2, 44, Canvas::Color::Medium, msg);
}
...
void Game::drawHUD(Canvas &canvas) {
    ...
    drawShadowText(canvas, 2, 7, Canvas::Color::Bright, level);

    FixedStringBuilder<16> score("Score %d", _player.score());
    drawShadowText(canvas, ScreenWidth - 2 - canvas.textWidth(score), 7, Canvas::Color::Bright, score);
}

void Game::drawShadowText(Canvas &canvas, int x, int y, Canvas::Color color, const char *str) {
